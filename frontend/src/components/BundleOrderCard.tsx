...
const getProgressPercent = (status: string) => {
  switch (status) {
    case "niitlegdsen": return 20;
    case "agent_sudlaj_bn": return 45;
    case "tolbor_huleej_bn": return 70;
    case "amjilttai_zahialga": return 100;
    case "tsutsalsan_zahialga": return 0;
    default: return 0;
  }
};
...
export const BundleOrderCard: React.FC<BundleOrderCardProps> = ({
  ...
  const canDelete = bundleOrder.status === "niitlegdsen" || bundleOrder.archivedByUser;
  ...
  const productNames = bundleOrder.items.map((item) => item.productName).join(", ");
  // Truncate if too long
  const displayName = productNames.length > 25
        ...
        <div className="flex items-center gap-3 min-w-0 flex-1">
          <span
            className={`w-2 h-2 rounded-full shrink-0 ${
              bundleOrder.status === "amjilttai_zahialga" ? "bg-green-500" :
              bundleOrder.status === "tsutsalsan_zahialga" ? "bg-red-500" :
              bundleOrder.status === "tolbor_huleej_bn" ? "bg-blue-500" :
              bundleOrder.status === "agent_sudlaj_bn" ? "bg-yellow-500" :
              "bg-gray-400"
            }`}
          />
          <div className="w-4 h-4 bg-purple-100 dark:bg-purple-900/30 rounded flex items-center justify-center shrink-0">
            <span className="text-[10px] font-bold text-purple-600 dark:text-purple-400">{bundleOrder.items.length}</span>
          </div>
          <span className="text-sm text-gray-900 dark:text-white truncate">{displayName}</span>
        </div>
        <button
          onClick={() => hasReport && onViewReport ? onViewReport(bundleOrder) : onViewDetails(bundleOrder)}
        ...
        <div className="flex-1 min-w-0">
          ...
          {bundleOrder.trackCode && (
            <div className="flex items-center gap-1.5 mt-1">
              <span className="text-xs text-gray-600 dark:text-gray-400 font-medium">Трак код:</span>
  ...
  return (
    <div className="relative rounded-2xl shadow-lg hover:shadow-xl hover:scale-[1.01] transition-all duration-300 overflow-hidden p-4 bg-gradient-to-br from-slate-800 via-slate-700 to-slate-900">
      {/* Decorative circles */}
      ...
      <div className="relative z-10 flex gap-4">
        ...
        {mainImage ? (
          ...
          <div className="w-20 h-20 shrink-0 bg-white/10 rounded-xl flex items-center justify-center relative ring-2 ring-white/20">
