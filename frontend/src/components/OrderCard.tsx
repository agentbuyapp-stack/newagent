    ...
    return (
      <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 hover:border-gray-300 dark:hover:border-gray-600 transition-all px-4 py-2.5 flex items-center justify-between gap-3">
        <div className="flex items-center gap-3 min-w-0 flex-1">
          <span
            className={`w-2 h-2 rounded-full shrink-0 ${order.status === "amjilttai_zahialga" ? "bg-green-500" :
              order.status === "tsutsalsan_zahialga" ? "bg-red-500" :
                order.status === "tolbor_huleej_bn" ? "bg-blue-500" :
                  order.status === "agent_sudlaj_bn" ? "bg-yellow-500" :
                    "bg-gray-400"
              }`}
          />
          <span className="text-sm text-gray-900 dark:text-white truncate">{order.productName}</span>
        </div>
        <button
          onClick={() => hasReport && onViewReport ? onViewReport(order) : onViewDetails(order)}
